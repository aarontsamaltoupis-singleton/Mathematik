...
Nach dem Cauchy-Kriterium kann  ein $M\in\mathbb{N} $ gewählt werden, sodass $$\forall n>m>M:\sum_{k=n}^m a_k<\frac{\varepsilon}{2} $$.\\
Sei $m>\max\{N,M\}$
\begin{align}
	\left|\sum_{n=1}^m a_{\tau (n)}-\sum^m_{n=1}a_n\right| &  &
	= \left|\sum^{m-d-1}_{n=1}a_n\ +\sum^{m}_{n=m-d}a_{ \tau (n)}-\sum^m_{n=1}a_n\right|                                                     \\
