...
\textbf{Hilfssatz 0}\\
Zu zeigen: $$
	C_m = & \sum^m_{n=0}b_n A_{m-n}\\$$
Beweis durch Induktion:\\
Induktionsanfang: $$C_0 = \sum^0_{n=0}c_n = a_0b_0 = \sum^0_{n=0}b_0A_0$$
Induktionsschritt:\\
Sei:
$C_m = & \sum^m_{n=0}b_nA_{m-n}$ \\
\begin{align*}
	C_{m+1}=        & \sum^{m+1}_{n=0}\sum^n_{k=0}a_kb_{n-k}                                  \\
	=               & \sum^{m}_{n=0}\sum_{k=0}^n a_kb_{n-k}+\sum_{n=0}^{m+1}a_{m+1-n} b_{n}   \\
	\overset{IH}{=} & \sum^m_{n=0} b_nA_{m-n} + a_{m+1-n}b_n + a_{0} b_{m+1}                  \\
	                & \sum^m_{n=0} b_nA_{m+1-n}+a_{0}b_{m+1}= \sum^{m+1}_{n=0}b_nA_{m+1} \qed
...
Da nach Hilfssatz 0 $C_m = \sum_{n=0}^m b_nA_{m-n}$,  gilt
$$
